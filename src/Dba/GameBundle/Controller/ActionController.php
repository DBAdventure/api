...
class ActionController extends BaseController
{
    ...
    public function postAttackAction(Player $target, $type = null)
    {
        ...
        if ($isDead) {
            ...
            $messages[] = 'action.kill.battlePoints';
     ...
     * @Annotations\Post("/talk/{quest}", name="_post")
     */
    public function getTalkAction(Request $request, Quest $quest)
    {
        $player = $this->getUser();
        ...
        $playerQuests = $player->getPlayerQuests()->filter(
            function ($entity) use ($quest) {
                return $entity->getQuest()->getId() == $quest->getId();
            }
        );
        $playerQuest = null;
        if (count($playerQuests) > 0) {
